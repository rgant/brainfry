...
export const ariaInvalidTest = (
  ...
  const inputEl: HTMLInputElement = safeQuerySelector(compiled, `#${fieldId}`);

  expect(control.invalid).withContext('FormControl.invalid').toBeTrue();
  expect(inputEl.getAttribute('aria-invalid')).withContext('get attribute aria-invalid').toBe('false');
  ...
  expect(inputEl.getAttribute('aria-invalid')).withContext('get attribute aria-invalid').toBe('true');
